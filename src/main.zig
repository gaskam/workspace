...
const command_definitions = @import("commands.zig").all;
...
const commandsMap = blk: {
    const kvs = build_kvs: {
        ...
        for (command_definitions) |definition| {
            kvs_array[i] = .{ definition.command.name, &definition.command };
            if (definition.command.alias) |alias| {
                kvs_array[i + 1] = .{ alias, &definition.command };
                i += 2;
            } else {
                i += 1;
            }
