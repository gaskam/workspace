...
function Install-Workspace {
  ...
  $DisplayVersion = $(
    ...
    elseif ($Version -eq "canary") { "Workspace Canary" }
    elseif ($Version -match "^workspace-v\d+\.\d+\.\d+$") { "Workspace $($Version.Substring(4))" }
  ...
  try {
    ...
    if (!(Test-Path "${WorkspaceBin}\$Target\workspace.exe")) {
      throw "The file '${WorkspaceBin}\$Target\workspace.exe' does not exist. Download is corrupt or intercepted Antivirus?`n"
    }
  } catch {
  ...
  Move-Item "${WorkspaceBin}\$Target\workspace.exe" "${WorkspaceBin}\workspace.exe" -Force

  Remove-Item "${WorkspaceBin}\$Target" -Recurse -Force
  Remove-Item $ZipPath -Force

  $WorkspaceRevision = "$(& "${WorkspaceBin}\workspace.exe" --revision)"
  ...
  { 
    ...
    Write-Output "The error above should be unreachable as Workspace does not depend on this library. Please comment in https://github.com/oven-sh/workspace/issues/8598 or open a new issue.`n`n"
    Write-Output "The command '${WorkspaceBin}\workspace.exe --revision' exited with code ${LASTEXITCODE}`n"
    return 1
  }
  ...
  try {
    $env:IS_WORKSPACE_AUTO_UPDATE = "1"
    # TODO: When powershell completions are added, make this switch actually do something
    if ($NoCompletions) {
      $env:WORKSPACE_NO_INSTALL_COMPLETIONS = "1"
    }
    $output = "$(& "${WorkspaceBin}\workspace.exe" completions 2>&1)"
    if ($LASTEXITCODE -ne 0) {
      Write-Output $output
      Write-Output "Install Failed - could not finalize installation"
      Write-Output "The command '${WorkspaceBin}\workspace.exe completions' exited with code ${LASTEXITCODE}`n"
      return 1
    }
  } catch {
    # it is possible on powershell 5 that an error happens, but it is probably fine?
  }
  $env:IS_WORKSPACE_AUTO_UPDATE = $null
  $env:WORKSPACE_NO_INSTALL_COMPLETIONS = $null

  $DisplayVersion = if ($WorkspaceRevision -like "*-canary.*") {
    "${WorkspaceRevision}"
  } else {
    "$(& "${WorkspaceBin}\workspace.exe" --version)"
  }
